...
<body>
    ...
    <form action="{{ route('alumnos.store') }}" method="POST" enctype="multipart/form-data" class="formulario">
        @csrf
        ...
        <div class="campo">
            ...
            <input type="number" name="nota_media" step="0.01" min="0" max="10" value="{{ old('nota_media') }}" required>
        ...
        <div class="campo">
            ...
            <textarea name="formacion">{{ old('formacion') }}</textarea>
            @error('formacion')<div class="error">{{ $message }}</div>@enderror
        </div>

        <div class="campo">
            <label>Habilidades</label>
            <textarea name="habilidades">{{ old('habilidades') }}</textarea>
